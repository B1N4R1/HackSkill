                    ...
                    if($issetSoft == 1){
                        ...
                        if(count($rowsReturned) != '1'){
                            ...
                            if($hddInfo['TOTAL'] >= $hddInfo['USED'] + $softInfo->softsize){
                                ...
                                $this->session->newQuery();
                                $sqlQuery = "INSERT INTO software (id, softHidden, softHiddenWith, softLastEdit, softName, softSize, softType, softVersion, userID, isNPC, originalFrom, licensedTo) VALUES (null, '0', '0', NOW(), ?, ?, ?, ?, ?, '0', ?, ?)";
                            ...
                            if($softInfo->softhidden != '1'){
                                ...
                                if($ramInfo['AVAILABLE'] - $softInfo->softram >= 0){
                                    ...
                                    if(!$this->software->isVirus($softInfo->softtype)){
                                        ...
                                        try {
                                          $pSoftID = intval($this->pSoftID);
                                          $this->session->newQuery();
                                          $sqlQuery = "INSERT INTO software_running (id, softID, userID, ramUsage, isNPC) VALUES ('', ?, ?, ?, ?)";
                                          $sqlReg = $this->pdo->prepare($sqlQuery);
                                          $sqlReg->execute(array($pSoftID, $id, $softInfo->softram, $this->pNPC));

                                        } catch (\Exception $e) {
                                          echo 'Excepción capturada: ',  $e->getMessage(), "\n";
                                          echo "<script>console.log(" . gettype(intval($this->pSoftID)) . " : " . intval($this->pSoftID) . ");</script>";
                                        }
    ...
    public function studyProcess($action, $host, $pcType){
            ...
            if(($this->software->issetSoftware($getInfoSoftID['GET_VALUE'], $id, $pcType)) || (strstr($action, 'XHD') !== FALSE)){
                ...
                if($host == 'local'){
                    ...
                    if($this->newProcess($_SESSION['id'], $action, 0, $host, $getInfoSoftID['GET_VALUE'], '', '', '0')) {
